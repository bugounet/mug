...
def joke_display_time_expired(start_time: datetime):
    return (
        int((datetime.now() - start_time).total_seconds()) >= JOKES_SELECTION_INTERVAL
    )
...
def display_joke():
    ...
    if (
        not last_joke_index
        or not joke_display_started_at
        or joke_display_time_expired(start_time=joke_display_started_at)
    ):
...
def print_joke(sentence: str, author="Denis"):
